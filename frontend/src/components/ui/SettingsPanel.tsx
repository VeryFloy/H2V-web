import { type Component, createSignal, createResource, createEffect, Show, For, lazy, onCleanup } from 'solid-js';
import { settingsStore, type AppSettings, type AutoDeleteMonths } from '../../stores/settings.store';
...
const SettingsPanel: Component<Props> = (props) => {
  ...
  const [showAutoDeletePicker, setShowAutoDeletePicker] = createSignal(false);
  ...
  return (
    <div class={styles.panel}>
      ...
      <div class={styles.header}>
        <BackBtn onClick={() => { if (subPageOpen()) closeSub(); else props.onClose(); }} />
          ...
          <div class={styles.body}>
            ...
            <Show when={renderedPage() === 'privacy'}>
              <div class={styles.subPage}>
                ...
                <div class={styles.row} onClick={() => setShowAutoDeletePicker((v) => !v)}>
                  <div class={styles.rowIcon}>
                  ...
                  <div class={styles.rowInfo}>
                    <div class={styles.rowLabel}>{t('privacy.auto_delete')}</div>
                    <div class={styles.rowDesc}>{t('privacy.auto_delete_desc')}</div>
                  </div>
                  <span class={styles.rowValue}>
                    {s().autoDeleteMonths === 'never' ? '' : t('privacy.auto_delete_months').replace('{n}', s().autoDeleteMonths)}
                  </span>
                </div>
                <Show when={showAutoDeletePicker()}>
                  <div class={styles.dropdownList}>
                    {(['1', '3', '6', '12'] as AutoDeleteMonths[]).map((val) => (
                      <button
                        class={`${styles.dropdownItem} ${s().autoDeleteMonths === val ? styles.dropdownItemActive : ''}`}
                        onClick={() => { set({ autoDeleteMonths: val }); setShowAutoDeletePicker(false); }}
                      >
                        {t('privacy.auto_delete_label').replace('{n}', val)}
                        <Show when={s().autoDeleteMonths === val}>
                ...
                <div class={styles.row} onClick={() => api.exportChat('all', 'html')}>
                  <div class={styles.rowIcon}>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4M7 10l5 5 5-5M12 15V3" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>
                  </div>
                  <div class={styles.rowInfo}>
                    <div class={styles.rowLabel}>{t('export.data')}</div>
                    <div class={styles.rowDesc}>{t('export.data_desc')}</div>
                  </div>
                </div>
        ...
        <div class={styles.dialogOverlay} onClick={() => setShowDeleteConfirm(false)}>
          <div class={styles.dialog} onClick={(e) => e.stopPropagation()}>
            ...
            <input
              ...
              placeholder={t('settings.delete_confirm_word')}
              value={deleteInput()}
            ...
            <div class={styles.dialogBtns}>
              ...
              <button
                ...
                disabled={deleteInput() !== t('settings.delete_confirm_word')}
                onClick={async () => {
                  try {
                    await api.deleteMe();
                    await authStore.logout();
